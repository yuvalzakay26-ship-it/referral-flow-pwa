...
/** Pure dashboard aggregation shared by the mock and Supabase implementations. */
export function computeDashboardStats(candidates: Candidate[]): DashboardStats {
  const c = candidates;
  ...
  return {
    ...
    followUpDue: count(
      (x) => Boolean(x.follow_up_date) && x.follow_up_date! <= today,
    ),
